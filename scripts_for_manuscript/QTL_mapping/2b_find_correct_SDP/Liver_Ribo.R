...
QTL_cat_easy = function(qtl_results, local_sdps)
{
	...
	local = apply(qtl_results, 1, function(x) 
	{
		...
		write(paste(x[2], new_local$chr, new_local$local_ID, sep="\t"), file = paste(dir, "20180213_Liver_Ribo/intermediate_sub_preprocess.txt", sep=""), append=TRUE)
...
#locals = QTL_cat_easy(eQTL_results_Liver_Ribo , local_sdps)

#save(locals, file =  paste(dir, "20180213_Liver_Ribo/intermediate_prepro.RData", sep=""))
load(file =  paste(dir, "20180213_Liver_Ribo/intermediate_prepro.RData", sep=""))
test = t(as.data.frame(locals))
ids_unknown = which(test[,1] == "-1")


save.image(paste(dir, "20180213_Liver_Ribo/image.RData", sep=""))
