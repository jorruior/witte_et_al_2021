...
source("/Volumes/huebner2/ANALYSES/20151127_qtl_RI_rn5/RI_translational_efficiency/scripts/R/eQTLpipeline_functions/eqtl_trans_kinship.R")
source("/Volumes/huebner2/ANALYSES/20151127_qtl_RI_rn5/RI_translational_efficiency/scripts/R//eQTLpipeline_functions/SeqQTL_oldCluster.R")
...
data_dir="/Volumes/huebner2/ANALYSES/20171204_RatHeartTranslatome/results/data_exploration/"
...
genotypes = read.table("/Volumes/huebner2/ANALYSES/20151127_qtl_RI_rn5/RI_translational_efficiency/data/20160318/RN6_SDPs/HXBBXH-SDP_RN60_LDB-genome.txt",  sep="\t", comment.char = "#", header=F, stringsAsFactors=F, colClasses="character", check.names=F)
colnames(genotypes) = c("ID", "RI_geno", "SDP_local")

####kick out single-marker SDPs
local_sdps = read.table("/Volumes/huebner2/ANALYSES/20151127_qtl_RI_rn5/RI_translational_efficiency/data/20160318/RN6_SDPs/HXBBXH-SDP_RN60_LDB-local.txt", sep="\t", comment.char = "#", header=F, stringsAsFactors=F, colClasses="character", check.names=F)
...
covar_lv_rna = read.table(paste(res_dir, "Input/covariates_sva_eQTL_lv.txt", sep=""), sep=" ", stringsAsFactors = F, header = T)
...
covar_lv_ribo = read.table(paste(res_dir, "Input/covariates_sva_rQTL_lv.txt", sep=""), sep=" ", stringsAsFactors = F, header = T)
...
covar_liver_rna = read.table(paste(res_dir, "Input/covariates_sva_eQTL_liver.txt", sep=""), sep=" ", stringsAsFactors = F, header = T)
...
covar_liver_ribo = read.table(paste(res_dir, "Input/covariates_sva_rQTL_liver.txt", sep=""), sep=" ", stringsAsFactors = F, header = T)
covar_liver_ribo = t(data.frame(id = colnames(covar_liver_ribo), t(covar_liver_ribo)))
write.table(covar_liver_ribo, file = paste(res_dir, "Input/covariates_sva_rQTL_liver_mod.txt", sep=""), sep="\t", col.names = F, row.names=T, quote = F)
...
covariates.file =  paste(res_dir, "Input/covariates_sva_eQTL_lv_mod.txt", sep="")
...
save(pheno, positions, snp.pos, genes, genotypes_alleles, genotypes_alleles, phenotype.file, genotypes.file, covariates.file,snps_location_file_name, gene_location_file_name, file = paste(res_dir, "Input/Lv_polyA/Lv_polyA_input_sva.RData", sep=""))
...
genotypes.file =  paste(res_dir, "Input/sdp_genotypes.txt", sep="")
covariates.file =  paste(res_dir, "Input/covariates_sva_eQTL_liver_mod.txt", sep="")
snps_location_file_name = paste(res_dir, "Input/snp_pos.txt", sep="")
...
save(pheno, positions, snp.pos, genes, genotypes_alleles, phenotype.file, genotypes.file, covariates.file,snps_location_file_name, gene_location_file_name, file = paste(res_dir, "Input/Liver_polyA/Liver_polyA_input_sva.RData", sep=""))
...
genotypes.file =  paste(res_dir, "Input/sdp_genotypes.txt", sep="")
covariates.file =  paste(res_dir, "Input/covariates_sva_rQTL_lv_mod.txt", sep="")
snps_location_file_name = paste(res_dir, "Input/snp_pos.txt", sep="")
gene_location_file_name = paste(res_dir, "Input/gene_pos_lv.txt", sep="")
positions = read.csv(gene_location_file_name, sep="\t", stringsAsFactors=F)
snp.pos = as.data.frame(fread(snps_location_file_name))
...
#covariates=covar_sva
genes = genes_filtered_lv 
save(pheno, positions, snp.pos, genes, genotypes_alleles, phenotype.file, genotypes.file, covariates.file,snps_location_file_name, gene_location_file_name, file = paste(res_dir, "Input/Lv_Ribo/Lv_ribo_input_sva.RData", sep=""))
...
genotypes.file =  paste(res_dir, "Input/sdp_genotypes.txt", sep="")
covariates.file =  paste(res_dir, "Input/covariates_sva_rQTL_liver_mod.txt", sep="")
snps_location_file_name = paste(res_dir, "Input/snp_pos.txt", sep="")
gene_location_file_name = paste(res_dir, "Input/gene_pos_liver.txt", sep="")
positions = read.csv(gene_location_file_name, sep="\t", stringsAsFactors=F)
snp.pos = as.data.frame(fread(snps_location_file_name))
...
#covariates=covar_sva
genes = genes_filtered_li 
save(pheno, positions, snp.pos, genes, genotypes_alleles, phenotype.file, genotypes.file, covariates.file,snps_location_file_name, gene_location_file_name, file = paste(res_dir, "Input/Liver_Ribo/Liver_ribo_input_sva.RData", sep=""))



#compute eQTLs
load(paste(res_dir, "Input/Lv_polyA/Lv_polyA_input_sva.RData", sep=""))
actual.eqtls = eqtl_new(phenotype.file, genotypes.file, covariates.file, positions, snp.pos, paste(res_dir, "Result_tables/20201117_Lv_polyA_sva/Lv_polyA", sep=""), compute.all=T, only_cis=F)
save(actual.eqtls, file = paste(res_dir, "Result_tables/20201117_Lv_polyA_sva/eqtls.RData", sep=""))

load(paste(res_dir, "Input/Liver_polyA/Liver_polyA_input_sva.RData", sep=""))
actual.eqtls = eqtl_new(phenotype.file, genotypes.file, covariates.file, positions, snp.pos, paste(res_dir, "Result_tables/20201119_Liver_polyA_sva/Liver_polyA", sep=""), compute.all=T, only_cis=F)
save(actual.eqtls, file=paste(res_dir, "Result_tables/20201119_Liver_polyA_sva/eqtls.RData", sep=""))

load(paste(res_dir, "Input/Lv_Ribo/Lv_ribo_input_sva.RData", sep=""))
actual.eqtls = eqtl_new(phenotype.file, genotypes.file, covariates.file, positions, snp.pos, paste(res_dir, "Result_tables/20201117_Lv_Ribo_sva/Lv_ribo", sep=""), compute.all=T, only_cis=F)
save(actual.eqtls, file=paste(res_dir, "Result_tables/20201117_Lv_Ribo_sva/eqtls.RData", sep=""))

load(paste(res_dir, "Input/Liver_Ribo/Liver_ribo_input.RData", sep=""))
actual.eqtls = eqtl_new(phenotype.file, genotypes.file, covariates.file, positions, snp.pos, paste(res_dir, "Result_tables/20201119_Liver_Ribo_sva/Liver_ribo", sep=""), compute.all=T, only_cis=F)
save(actual.eqtls, file=paste(res_dir, "Result_tables/20201119_Liver_Ribo_sva/eqtls.RData", sep=""))

########### TE - use residuals of lm(ribo~rna) ########### 


covar_lv_te = read.table(paste(res_dir, "Input/covariates_sva_teQTL_lv.txt", sep=""), sep=" ", stringsAsFactors = F, header = T)
covar_lv_te = t(data.frame(id = colnames(covar_lv_te), t(covar_lv_te)))
write.table(covar_lv_te, file = paste(res_dir, "Input/covariates_sva_teQTL_lv_mod.txt", sep=""), sep="\t", col.names = F, row.names=T, quote = F)
...
genotypes.file =  paste(res_dir, "Input/sdp_genotypes.txt", sep="")
covariates.file =  paste(res_dir, "Input/covariates_sva_teQTL_lv_mod.txt", sep="")
snps_location_file_name = paste(res_dir, "Input/snp_pos.txt", sep="")
gene_location_file_name = paste(res_dir, "Input/gene_pos_lv.txt", sep="")
positions = read.csv(gene_location_file_name, sep="\t", stringsAsFactors=F)
snp.pos = as.data.frame(fread(snps_location_file_name))
#covariates=covar_sva
genes = genes_filtered_lv 
save(pheno, positions, snp.pos, genes, genotypes_alleles, genotypes_alleles, phenotype.file, genotypes.file, covariates.file,snps_location_file_name, gene_location_file_name, file = paste(res_dir, "Input/Lv_Ribo_RNA_resid/Lv_Ribo_RNA_resid_input_sva.RData", sep=""))


load(paste(res_dir, "Input/Lv_Ribo_RNA_resid/Lv_Ribo_RNA_resid_input_sva.RData", sep=""))
actual.eqtls = eqtl_new(phenotype.file, genotypes.file, covariates.file, positions, snp.pos, paste(res_dir, "Result_tables/20201117_Lv_Ribo_RNA_resid_sva/Lv_Ribo_RNA_resid_", sep=""), compute.all=T, only_cis=F)
save(actual.eqtls, file=paste(res_dir, "Result_tables/20201117_Lv_Ribo_RNA_resid_sva/eqtls.RData", sep=""))
...
genotypes.file =  paste(res_dir, "Input/sdp_genotypes.txt", sep="")
covariates.file = paste(res_dir, "Input/covariates_sva_teQTL_liver.txt", sep="")
snps_location_file_name = paste(res_dir, "Input/snp_pos.txt", sep="")
gene_location_file_name = paste(res_dir, "Input/gene_pos_liver.txt", sep="")
positions = read.csv(gene_location_file_name, sep="\t", stringsAsFactors=F)
snp.pos = as.data.frame(fread(snps_location_file_name))
#covariates=NULL
genes = genes_filtered_li 
save(pheno, positions, snp.pos, genes, genotypes_alleles, genotypes_alleles, phenotype.file, genotypes.file, covariates.file,snps_location_file_name, gene_location_file_name, file = paste(res_dir, "Input/Liver_Ribo_RNA_resid/Liver_Ribo_RNA_resid_input.RData", sep=""))
