...
load(paste(dir, "20180213_Lv_Ribo/image.RData", sep=""))
...
for(counter in c(1:24))
{
	...
	load(file =  paste(dir, "20180213_Lv_Ribo/unknowns_", counter, ".RData", sep=""))
...
kind=rep("NA", nrow(eQTL_results_Lv_Ribo))
setkey(eQTL_results_Lv_Ribo,gene,chrom,snp_pos)

kind[eQTL_results_Lv_Ribo[[9]] != test[,1]] = "trans"
kind[eQTL_results_Lv_Ribo[[9]] == test[,1]] = "cis"
kind[eQTL_results_Lv_Ribo[[1]] == eQTL_results_Lv_Ribo[[14]]] = "cis"

eQTL_results_Lv_Ribo$kind = kind
eQTL_results_Lv_Ribo$chr_sdp_test = test[,1]
eQTL_results_Lv_Ribo$sdp_test = test[,2]
setkey(eQTL_results_Lv_Ribo, kind)
eQTL_trans = eQTL_results_Lv_Ribo[kind == "trans"]
eQTL_cis = eQTL_results_Lv_Ribo[kind == "cis"]
...
write.table(eQTL_trans.sign, file="/data/huebner2/ANALYSES/20171204_RatHeartTranslatome/results/QTL_mapping/final_table/distant_eQTLs_Lv_Ribo_sign.txt", sep="\t", col.names=T, row.names=F, quote=F)

eQTL_cis.sign = eQTL_cis[FDR_new <= 0.05]
#6960 pairs
#1269 unique genes
setkey(eQTL_cis.sign, gene)
write.table(eQTL_cis.sign, file="/data/huebner2/ANALYSES/20171204_RatHeartTranslatome/results/QTL_mapping/final_table/local_eQTLs_Lv_Ribo_sign.txt", sep="\t", col.names=T, row.names=F, quote=F)

save.image("/data/huebner2/ANALYSES/20171204_RatHeartTranslatome/results/QTL_mapping/final_table/Lv_Ribo_tables.RData")
